...
transformAMap {f} {g} {h} {x} =
  ...
  (transformA (pure {f} h <*> x))
    ={ transformAAp {x = pure {f} h} {y=x} }=
  (transformA (pure {f} h) <*> transformA x)
    ={ cong {f=\z=>z <*> transformA x} $ transformAPure {x=h} }=
  (pure {f=g} h <*> transformA x)
    ={ cong {f=\z=>z (transformA x)} $ sym $ applicativeMap {u=h} }=
  (map {f=g} h (transformA x))
    QED
